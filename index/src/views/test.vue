...
<template>
  <div class="yu">
    ...
    <!-- 菜单开始 -->
    <div class="div_img1">
      <div v-for="(item, index) of tooth_a" :key="index">
        <div>
          <img
            :src="`http://127.0.0.1:3000/img/_wzy/${item.tooth_img}`"
            alt=""
          />
        </div>
        <div class="div_xiya">
...
<style scoped>
/* 顶部选项卡*/
.yu .xianxiang > div > div {
  background-color: transparent;
}
.yu .xianxiang .van-tabs__wrap .van-tabs__nav .van-tabs__line {
...
.yu .jianjin1 div:not(:first-child) {
  text-indent: 0;
}
.yu .jianjin1_1 {
  overflow: hidden;
...
export default {
  ...
  mounted() {
    this.axios.get("/tooth?cid=" + 1).then((res) => {
    ...
    this.axios.get("/tooth_a?cid=" + this.active).then((res) => {
      ...
      //      item.tooth_img=require("http://127.0.0.1:3000/img/_wzy/"+item.tooth_img)
      //    }
